"""Isolation: Stage 2 must not touch brokers, orders, or the hourly agent."""
...
def _src_root() -> Path:
    return Path(__file__).resolve().parents[1] / "src" / "northstar_mean_reversion"
...
def test_stage2_source_has_no_broker_or_order_api():
    hits: list[str] = []
...
def test_hourly_agent_and_rules_do_not_import_stage2():
    root = _repo_root()
    watched = [
